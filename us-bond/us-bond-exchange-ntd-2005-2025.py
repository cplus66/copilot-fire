...
end_date = '2025-01-01'
df_bond_yield = df_bond_yield[(df_bond_yield.index >= start_date) & (df_bond_yield.index <= end_date)]
df_usd_ntd = df_usd_ntd[(df_usd_ntd.index >= start_date) & (df_usd_ntd.index <= end_date)]

# Resample the data to quarterly frequency, taking the last value of each period
df_bond_yield_quarterly = df_bond_yield.resample('QE').last()
df_usd_ntd_quarterly = df_usd_ntd.resample('QE').last()
...
plt.title('Historical Quarterly U.S. 10-Year Bond Yield Rate and USD to NTD Exchange Rate (2005-2025)')
plt.show()
