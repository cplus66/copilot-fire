...
if [ "$COMMAND" == "add" ]; then
  ...
  jq --arg inst "$ARG" 'if .INSTANCES | index($inst) then . else .INSTANCES += [$inst] end' "$CONFIG_FILE" > \
	  tmp.json && mv tmp.json "$CONFIG_FILE"
  ...
  jq --argjson new_instances "$(jq -R . < $ARG | jq -s .)" '.INSTANCES = $new_instances' "$CONFIG_FILE" > \
	  tmp.json && mv tmp.json "$CONFIG_FILE"
